...
def test_user_registration(client):
    ...
    expected_keys = ["id", "username", "email", "first_name", "last_name", "auth_token"]
...
def test_user_registration_without_name(client):
    url = reverse("auth-register")
    credentials = {"username": "test", "email": "test@test.com", "password": "localhost"}
    response = client.json.post(url, json.dumps(credentials))
    assert response.status_code == 201
    expected_keys = ["id", "username", "email", "first_name", "last_name", "auth_token"]
    assert set(expected_keys).issubset(response.data.keys())
...
def test_user_login(client):
    ...
    u = f.create_user(username="test", email="test@example.com", password="test")
...
def test_user_password_change(client):
    ...
    user = f.create_user(username="test", email="test@example.com", password=current_password)
...
def test_user_password_reset(client, mailoutbox, settings):
    ...
    user = f.create_user(username="test", email="test@example.com")
...
def test_user_password_reset_and_confirm(client, settings, mocker):
    url = reverse("auth-password-reset")
    user = f.create_user(username="test", email="test@example.com")
    mock_email = mocker.patch("apps.users.auth.services.send_mail")
