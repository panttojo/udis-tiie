...
def test_patch_current_user_api(client):
    url = reverse("me")
    user = f.create_user(username="username", email="test@example.com", first_name="test", last_name="test")

    data = {
        "username": "modified_test",
    ...
    assert response.data["username"] == "modified_test"
